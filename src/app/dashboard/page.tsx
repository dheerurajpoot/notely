...
import { getOrdersByBuyer, getProductsBySeller } from "@/lib/db"
import { UserRole } from "@/lib/models"
import { StatsCard } from "@/components/stats-card"

export default function DashboardPage() {
  const user = requireAuth()
  const purchases = getOrdersByBuyer(user?.id)
  const products = user?.role === UserRole.SELLER || user?.role === UserRole.ADMIN ? getProductsBySeller(user?.id) : []
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        {(user?.role === UserRole.SELLER || user?.role === UserRole.ADMIN) && (
          <>
            ...
            />
            <StatsCard title="Total Customers" value="24" icon={<Users className="h-4 w-4 text-muted-foreground" />} />
          </>
        )}
      </div>
        ...
        <TabsContent value="recent" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your most recent purchases on Notely</CardDescription>
            </CardHeader>
            <CardContent>
              {purchases.length > 0 ? (
          ...
          {(user?.role === UserRole.SELLER || user?.role === UserRole.ADMIN) && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your most recent sales on Notely</CardDescription>
              </CardHeader>
              <CardContent>
